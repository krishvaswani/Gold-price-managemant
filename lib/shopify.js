...
let resolvedDomain = SHOPIFY_DOMAIN;
const getBaseUrl = () => `https://${resolvedDomain}/admin/api/2026-01`;
...
// Helper to resolve custom domain redirection to .myshopify.com
async function resolveDomainIfNeeded() {
  if (SHOPIFY_DOMAIN && !SHOPIFY_DOMAIN.includes('.myshopify.com') && resolvedDomain === SHOPIFY_DOMAIN) {
    try {
      const res = await fetch(`https://${SHOPIFY_DOMAIN}/admin/api/2026-01/shop.json`, { headers });
      const finalUrl = new URL(res.url);
      if (finalUrl.hostname.includes('.myshopify.com')) {
        resolvedDomain = finalUrl.hostname;
        console.log(`[Shopify API] Resolved custom domain ${SHOPIFY_DOMAIN} to ${resolvedDomain}`);
      }
    } catch (e) {
      console.error(`[Shopify API] Failed to resolve custom domain:`, e);
    }
  }
}
...
export async function getShopMetafields() {
  await resolveDomainIfNeeded();
  const res  = await fetch(`${getBaseUrl()}/metafields.json?namespace=custom`, { headers });
...
export async function setShopMetafield(key, value, type = 'number_decimal') {
  ...
  if (existing) {
    await fetch(`${getBaseUrl()}/metafields/${existing.id}.json`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ metafield: { id: existing.id, value: String(value), type } }),
    });
  } else {
    await fetch(`${getBaseUrl()}/metafields.json`, {
      method: 'POST',
      headers,
...
async function getShopMetafieldByKey(key) {
  const res  = await fetch(`${getBaseUrl()}/metafields.json?namespace=custom&key=${key}`, { headers });
  const data = await res.json();
...
export async function getAllProducts() {
  await resolveDomainIfNeeded();
  let products = [];
  let url      = `${getBaseUrl()}/products.json?limit=250&fields=id,title,variants`;
...
export async function getAllProductsWithMetafields() {
  ...
  const GRAPHQL_URL = `https://${resolvedDomain}/admin/api/2026-01/graphql.json`;
...
export async function getProductMetafields(productId) {
  const res  = await fetch(`${getBaseUrl()}/products/${productId}/metafields.json?namespace=custom`, { headers });
  const data = await res.json();
  const map  = {};
...
export async function getVariantMetafields(variantId) {
  const res  = await fetch(`${getBaseUrl()}/variants/${variantId}/metafields.json?namespace=custom`, { headers });
  const data = await res.json();
  const map  = {};
  (data.metafields || []).forEach(m => { map[m.key] = m.value; });
  return map;
...
export async function updateVariantPrice(variantId, price) {
  ...
  while (attempt < maxRetries) {
    const res = await fetch(`${getBaseUrl()}/variants/${variantId}.json`, {
      method: 'PUT',
      headers,
